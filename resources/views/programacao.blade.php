                        ...
                        @foreach ($periodo as $key => $date)
                            <div class="tab-pane fade show schedule-item {{ ($key == 0) ? 'active' : '' }}" id="box{{ $key+1 }}">
                                <ul class="m-0 p-0">
                                    ...
                                    @foreach($atividades->whereBetween('dt_inicio_atividade_ati', [$date->format('Y-m-d'), $date->addDays(1)->format('Y-m-d')])->sortBy('dt_inicio_atividade_ati','nm_atividade_ati') as $atividade)
